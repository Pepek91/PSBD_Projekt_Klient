...
public class SiecHoteli {
    ...
    public void addPokoj(int id, int nrPokoju, float cena, short standard, short maxOsob, short liczbaPomieszczen, short liczbaGwiazdek, String nazwa, int idHotelu) {
        int iloscHoteli = listaHoteli.size();

        for (int i = 0; i < iloscHoteli; ++i) {
            if (listaHoteli.get(i).getIdHotelu() == idHotelu) {
                listaHoteli.get(i).addPokojDoHotelu(id, nrPokoju, cena, standard, maxOsob, liczbaPomieszczen);
                return;
            }
        }
        Hotel hotel = new Hotel(liczbaGwiazdek, nazwa, idHotelu, parentFrame);
